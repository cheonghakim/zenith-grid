...
export class HeaderRenderer {
  ...
  _renderContainer(container, leafColumns, pinArea, horizontalRange) {
    ...
    renderColumns.forEach(({ def, state }, index) => {
      row.appendChild(
        this._buildSingleLevelCell(def, state, index, stickyMeta, leafColumns),
      );
    });
  ...
  _renderMultiLevel(
    ...
    const leafWidthMap = new Map(
      leafColumns.map(({ def, state }) => [def.id, state.width ?? 0]),
    );
    ...
    for (let depthIdx = 0; depthIdx < totalDepths; depthIdx++) {
      ...
      const orderedCells = this._buildDepthRowSequence(
        depthIdx,
        headerRows,
        pinLeafIds,
      );
  ...
  _buildDepthRowSequence(depthIdx, headerRows, pinLeafIds) {
    if (depthIdx === 0) {
      return headerRows[0].filter((cell) =>
        this._cellBelongsToArea(cell, pinLeafIds),
      );
    }

    const result = [];
    const prevSeq = this._buildDepthRowSequence(
      depthIdx - 1,
      headerRows,
      pinLeafIds,
    );
  ...
  _cellBelongsToArea(cell, pinLeafIds) {
    if (cell._isSpanPlaceholder || !cell.isGroup)
      return pinLeafIds.has(cell.colId);
    return this._groupLeafIds(cell.def).some((id) => pinLeafIds.has(id));
  }

  _cellIsVisible(cell, visibleLeafIds) {
    if (cell._isSpanPlaceholder || !cell.isGroup)
      return visibleLeafIds.has(cell.colId);
  ...
  _buildMultiLevelCell(cell, pinLeafIds, leafWidthMap, rowHeight) {
    ...
    if (cell.def.filterable !== false) {
      domCell.appendChild(
        this._createFilterAction(cell.colId, cell.def, domCell),
      );
    }
  ...
  _buildSingleLevelCell(def, state, index, stickyMeta, allColumns) {
    ...
    button.setAttribute(
      "aria-label",
      `${def.headerName}${sortDef ? `, sorted ${sortDef.direction}ending` : ""}`,
    );
    button.dataset.gridFocusable = "header";
  ...
  _createSelectionHeaderCell() {
    ...
    checkbox.indeterminate =
      !checkbox.checked && Boolean(this._options.isSomeSelected?.());
  ...
  _createFilterAction(colId, def, anchorCell) {
    ...
    button.setAttribute(
      "aria-expanded",
      this._openFilterColId === colId ? "true" : "false",
    );
  ...
  _buildStickyMeta(columns, pinArea) {
    ...
    const selectionOffset =
      pinArea === "left" && this._options.selectionEnabled
  ...
  _getSortState(colId) {
    ...
    if (!sortManager) return null;
    const sort = sortManager.getSortForField(def?.field ?? colId);
    if (!sort) return null;
    const state = sortManager.getState();
    const priority =
      state.sortDefs.length > 1
        ? String(
            state.sortDefs.findIndex((s) => s.field === (def?.field ?? colId)) +
              1,
          )
        : "";
    return {
  ...
  _renderSortIndicator(indicator, colId) {
    indicator.textContent = "";
    const sort = this._getSortState(colId);
    if (!sort) return;
  ...
  _renderFilterRow(container, leafColumns, pinArea, horizontalRange) {
    ...
    const extraWidth =
      (this._options.rowNumbers ? (this._options.rowNumberWidth ?? 44) : 0) +
      (this._options.selectionEnabled
        ? (this._options.selectionColumnWidth ?? 44)
        : 0);
    ...
    let renderColumns = leafColumns;
    if (pinArea === "center" && horizontalRange) {
      renderColumns = this._sliceColumnsByRange(
        leafColumns,
        horizontalRange,
      ).columns;
    }
  ...
  _renderFilterPopover(colId, def, anchorCell, trigger) {
    ...
    if (meta.type !== "select" && meta.type !== "set") {
      ...
      const editor = this._createFilterEditor(
        meta,
        operator,
        value,
        (nextValue) => {
      ...
      selectAllText.textContent = this._getLocaleText(
  ...
  _handleDocumentPointerDown(event) {
    ...
    if (
      this._filterPopover.element.contains(target) ||
      this._filterPopover.trigger?.contains(target)
    ) {
      return;
    }
  ...
  _createFilterEditor(meta, operator, value, onCommit) {
    if (operator === "between") {
      ...
      const endInput = this._createFilterInput(
        meta,
      ...
      startInput.placeholder = this._getLocaleText(
        "sidePanel.rangeStart",
        "From",
      );
    ...
    const input = this._createFilterInput(
      meta,
  ...
  _createFilterInput(meta, value, onCommit) {
    ...
    input.addEventListener(
      meta.inputType === "search" ? "input" : "change",
      commit,
    );
  ...
  _resolveFilterMeta(def, colId) {
    const type =
      def.filterType ??
      (Array.isArray(def.filterOptions) && def.filterOptions.length > 0
        ? "select"
        : def.type === "number" || def.type === "date"
          ? def.type
          : "text");
    const operators =
      Array.isArray(def.filterOperators) && def.filterOperators.length > 0
        ? def.filterOperators
    ...
    return {
      ...
      inputType:
        type === "number" ? "number" : type === "date" ? "date" : "search",
      placeholder:
        def.filterPlaceholder ??
        this._getLocaleText("sidePanel.filterPlaceholder", "Filter {label}", {
          label: def.headerName,
        }),
      choices:
        type === "select" || type === "set"
          ? (this._options.getColumnFilterChoices?.(colId) ?? [])
          : [],
    };
  }
  ...
  _getOperatorLabel(operator) {
    const labels = {
      ...
      startsWith: this._getLocaleText(
        "sidePanel.operators.startsWith",
        "Starts With",
      ),
      endsWith: this._getLocaleText(
        "sidePanel.operators.endsWith",
        "Ends With",
      ),
      equals: this._getLocaleText("sidePanel.operators.equals", "Equals"),
      notEquals: this._getLocaleText(
        "sidePanel.operators.notEquals",
        "Does Not Equal",
      ),
      notContains: this._getLocaleText(
        "sidePanel.operators.notContains",
        "Does Not Contain",
      ),
      greaterThan: this._getLocaleText(
        "sidePanel.operators.greaterThan",
        "Greater Than",
      ),
      greaterThanOrEqual: this._getLocaleText(
        "sidePanel.operators.greaterThanOrEqual",
        "Greater Or Equal",
      ),
      lessThan: this._getLocaleText(
        "sidePanel.operators.lessThan",
        "Less Than",
      ),
      lessThanOrEqual: this._getLocaleText(
        "sidePanel.operators.lessThanOrEqual",
        "Less Or Equal",
      ),
  ...
  _createFilterIcon() {
    return createSvgIcon("filterVariant", 14);
  }
  ...
  _closeHeaderContextMenu() {
    ...
    if (this._dismissHeaderContextMenu) {
      document.removeEventListener(
        "pointerdown",
        this._dismissHeaderContextMenu,
        true,
      );
