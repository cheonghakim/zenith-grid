...
**CSS 격리:** HighGrid는 모든 내부 CSS 클래스에 `ck-high-grid-` 프리픽스를, CSS 커스텀 프로퍼티에 `--ck-high-grid-` 프리픽스를 사용합니다. AG-Grid 등 다른 그리드 라이브러리와 같은 페이지에서 사용해도 CSS 충돌이 발생하지 않습니다.
...
**Vue CLI / Webpack 사용자 주의:** `Cannot find module 'highgrid/styles/grid.css'` 에러가 발생하면 직접 경로를 사용하세요:
...
<!-- 컴팩트 (32px 행) -->
<div class="ck-high-grid-theme-compact">
  <div id="my-grid"></div>
</div>

<!-- 스패이셔스 (52px 행) -->
<div class="ck-high-grid-theme-spacious">
  <div id="my-grid"></div>
</div>
```
...
`printGrid`는 `window.print()`를 호출합니다. `@media print` CSS로 출력 레이아웃을 커스터마이즈할 수 있으며, HighGrid는 스크롤바와 오버레이를 자동으로 숨깁니다.
