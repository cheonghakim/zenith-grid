...
describe('GridCore DOM smoke', () => {
  ...
  it('renders rows, supports HTMLElement cell renderers, and updates the side panel state', async () => {
    ...
    await flush();
    await vi.waitFor(() => {
      expect(host.querySelectorAll('.ck-zenith-grid-row').length).toBeGreaterThan(0);
    });
  ...
  it('shows tree toggles immediately when tree mode is enabled', async () => {
    ...
    await vi.waitFor(() => {
      expect(host.querySelector('.ck-zenith-grid-row-tree .ck-zenith-grid-row-toggle')).not.toBeNull();
    });

    expect(host.querySelector('.ck-zenith-grid-row-kind-tree')).not.toBeNull();
  ...
  it('lets callers disable row selection and decorate rows through simple options', async () => {
    ...
    const lockedRow = host.querySelector('.ck-zenith-grid-row[data-row-key="locked"]');
    const lockedCheckbox = lockedRow?.querySelector('.ck-zenith-grid-selection-checkbox');
  ...
  it('renders richer per-column filter controls and marks filtered headers', async () => {
    ...
    expect(host.querySelector('.ck-zenith-grid-side-panel-select-multiple')).not.toBeNull();
    expect(grid.getColumnFilterChoices('status').map((entry) => entry.value)).toEqual(['Active', 'Paused']);
    expect(host.querySelector('.ck-zenith-grid-header-filter-button')).not.toBeNull();

    grid.setColumnFilter('status', { type: 'select', field: 'status', operator: 'in', value: ['Active'] });
    await flush();

    expect(host.querySelector('.ck-zenith-grid-header-cell-filtered[data-col-id="status"]')).not.toBeNull();
  ...
  it('opens a header filter popover from the column action button', async () => {
    ...
    host.querySelector('.ck-zenith-grid-header-filter-button')?.dispatchEvent(new MouseEvent('click', { bubbles: true }));
    await flush();

    expect(document.body.querySelector('.ck-zenith-grid-header-filter-popover')).not.toBeNull();
  ...
  it('renders custom loading, empty, and error overlays', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(host.querySelector('.ck-zenith-grid-overlay-action')?.textContent).toBe('Retry');
    });
  ...
  it('keeps filter popovers scoped to the correct grid and closes them on escape', async () => {
    ...
    secondHost.querySelector('.ck-zenith-grid-header-filter-button')?.dispatchEvent(new MouseEvent('click', { bubbles: true }));
    await flush();

    const popover = document.body.querySelector('.ck-zenith-grid-header-filter-popover');
    expect(popover).not.toBeNull();

    document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true }));
    await flush();

    expect(document.body.querySelector('.ck-zenith-grid-header-filter-popover')).toBeNull();
  ...
  it('exports visible rows to csv and supports context-menu plugins', async () => {
    ...
    host.querySelector('.ck-zenith-grid-cell[data-col-id="name"]')?.dispatchEvent(new MouseEvent('contextmenu', {
      bubbles: true,
      cancelable: true,
      clientX: 120,
      clientY: 80,
    ...
    expect(document.body.querySelector('.ck-zenith-grid-context-menu-item')?.textContent).toContain('Inspect Alpha');
  ...
  it('edits cells, validates values, and exposes clipboard/export helpers', async () => {
    ...
    expect(host.querySelector('.ck-zenith-grid-cell-invalid[data-col-id="score"]')).not.toBeNull();
  ...
  it('renders select, date, and textarea editors and supports autoSizeColumn/autoSizeAllColumns', async () => {
    ...
    await flush();
    let cell = host.querySelector('.ck-zenith-grid-cell[data-col-id="team"]');
    let editor = cell?.querySelector('.ck-zenith-grid-cell-editor');
    ...
    await flush();
    cell = host.querySelector('.ck-zenith-grid-cell[data-col-id="joined"]');
    editor = cell?.querySelector('.ck-zenith-grid-cell-editor');
    ...
    await flush();
    cell = host.querySelector('.ck-zenith-grid-cell[data-col-id="note"]');
    editor = cell?.querySelector('.ck-zenith-grid-cell-editor');
  ...
  it('renders Set Filter checkbox list with search and filters rows accordingly', async () => {
    ...
    const selectAllCheck = popover.querySelector('.ck-zenith-grid-filter-checkbox'); // first one is Select All
    const checkboxes = popover.querySelectorAll('.ck-zenith-grid-filter-list-container .ck-zenith-grid-filter-checkbox');
    ...
    const blueCheck = [...popover.querySelectorAll('.ck-zenith-grid-filter-checkbox-label')].find(el => el.textContent === 'Blue')?.querySelector('input');
    const greenCheck = [...popover.querySelectorAll('.ck-zenith-grid-filter-checkbox-label')].find(el => el.textContent === 'Green')?.querySelector('input');
    ...
    const visibleLabels = [...popover.querySelectorAll('.ck-zenith-grid-filter-checkbox-label')].map(el => el.textContent);
  ...
  it('supports row dragging and cell spanning (rowSpan/colSpan)', async () => {
    ...
    const dragHandle = host.querySelector('.ck-zenith-grid-cell[data-col-id="name"] .ck-zenith-grid-row-drag-handle');
    expect(dragHandle).not.toBeNull();

    // 2. Verify row reordering on drop
    const firstRowEl = host.querySelector('.ck-zenith-grid-row[data-row-key="1"]');
    const thirdRowEl = host.querySelector('.ck-zenith-grid-row[data-row-key="3"]');
    ...
    const cellSpanned = host.querySelector('.ck-zenith-grid-row[data-row-key="10"] .ck-zenith-grid-cell[data-col-id="score"]');
    const cellSkipped = host.querySelector('.ck-zenith-grid-row[data-row-key="20"] .ck-zenith-grid-cell[data-col-id="score"]');
  ...
  it('blocks editing on primary key column and ignores arrow keys in editor', async () => {
    ...
    const cell = host.querySelector('.ck-zenith-grid-cell[data-col-id="name"]');
    const editor = cell?.querySelector('.ck-zenith-grid-cell-editor');
    ...
    expect(cell.classList.contains('ck-zenith-grid-cell-editing')).toBe(true);
    expect(cell.querySelector('.ck-zenith-grid-cell-editor')).not.toBeNull();
