...
test.describe('HighGrid demo smoke', () => {
  test('renders the main demo and responds to common remote actions', async ({ page }) => {
    ...
    await expect(page.locator('#gridApp .ag-row').first()).toBeVisible();
    ...
    await page.getByRole('button', { name: '리로드 측정' }).click();
  ...
  test('keeps the floating controls keyboard-reachable and labeled', async ({ page }) => {
    ...
    await page.getByRole('button', { name: '스트림 시작' }).focus();
    await expect(page.getByRole('button', { name: '스트림 시작' })).toBeFocused();
