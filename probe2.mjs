...
// root background
const rootBg = await page.$eval('#grid-dark .ck-zenith-grid-root', el => {
  const s = window.getComputedStyle(el);
  return { bg: s.backgroundColor, bgImage: s.backgroundImage };
});

// pagination footer
const footer = await page.$eval('#grid-light .ck-zenith-grid-footer', el => {
  const s = window.getComputedStyle(el);
...
// dark theme — get the ck-zenith-grid-root actual class list  
const rootClass = await page.$eval('#grid-dark .ck-zenith-grid-root', el => el.className);
